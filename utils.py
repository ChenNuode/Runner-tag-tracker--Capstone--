...
def rects2polys(rects, thetas, origins, framewidth, frameheight, ratioWidth=1, ratioHeight=1, padding=0.0):
    ...
    for i, box in enumerate(rects):
        ...
        #get padding
        paddingx =  int((lowerRightX - upperLeftX)  * padding)
        paddingy = int((lowerRightX - upperLeftX)  * padding)

        # scale points to padding
        upperLeftX = max(0, upperLeftX - paddingx)
        upperLeftY = max(0, upperLeftY - paddingy)
        lowerRightX = min(framewidth, lowerRightX + paddingx)
        lowerRightY = min(frameheight, lowerRightY + paddingy)
